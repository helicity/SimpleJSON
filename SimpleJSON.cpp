...
namespace
{
    ...
    void consume_ws(const string& str, size_t& offset)
    {
        // skip '#' style comment
repeat:
        while (isspace(str[offset]) && str[offset] != '#') ++offset;
        if (str[offset] == '#')
        {
            while (str[offset] != '\n' && str[offset] != '\r') ++offset;
            while (str[offset] == '\n' || str[offset] == '\r') ++offset;
...
{
	if (!IsArray()) return JSON_NULL;
	if (index >= Internal.List->size()) return JSON_NULL;
    return Internal.List->operator[](index);
}
